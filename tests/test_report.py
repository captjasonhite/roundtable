...
class ReportRenderTests(unittest.TestCase):
    ...
    def test_judging_documents_are_linked_from_the_verdicts(self):
        """The page says what the panel concluded; these are what from."""
        self.write("01_x_alpha_thinking.md", RUN)
        self.write("02_x_beta_thinking.md", RUN_B)
        self.write("SUMMARIZE-KEY.md", KEY_TWO)
        self.write("SUMMARIZE.md", "# what the judges read\n")
        self.write("summary_20260101-000000_gamma.md", VERDICT)
        _, html = self.render()
        self.assertIn('href="SUMMARIZE.md"', html)
        self.assertIn('href="SUMMARIZE-KEY.md"', html)
        self.assertIn("What the judges were shown", html)

    def test_a_missing_judging_document_is_not_linked(self):
        """An older session may have no SUMMARIZE.md; a dead link is worse
        than no link."""
        self.write("01_x_alpha_thinking.md", RUN)
        self.write("02_x_beta_thinking.md", RUN_B)
        self.write("SUMMARIZE-KEY.md", KEY_TWO)
