...
_CHAIN_CSS = """
.stage-h{font-size:20px;margin:54px 0 18px;padding-top:26px;
  border-top:2px solid var(--border);letter-spacing:-.01em}
.stage-h:first-of-type{margin-top:30px}
.stage-h .n{color:var(--muted);font-weight:400;font-size:14px;margin-right:8px}
"""


def render_chain(name, stage_entries, running=False, refresh=15, status_note=""):
    """-> complete HTML document for a chain: every stage rendered with the
    exact same sections a plain session's report gets, one stack after
    another in run order -- a chain is more rounds of the same report, not a
    different kind of page. The last stage's own analysis (its
    ``_meta_summary``, if that stage asked for one) is what answers "what do
    the final results say," the same as it would on a plain session's report
    -- nothing extra is invented for it.

    ``stage_entries`` -- ``[(stage_name, session_or_None, result_or_None,
    rankings_or_None, state)]`` in run order, ``state`` one of
    ``"done"``, ``"running"``, ``"queued"``.
    """
    stamp = time.strftime("%Y-%m-%d %H:%M:%S")
    head = ["<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\">",
    ...
    if running:
        head.append('<meta http-equiv="refresh" content="%d">' % refresh)
    head.append("<title>%s — creative-bench</title>" % esc(name))
    head.append('<link rel="icon" type="image/png" sizes="32x32" '
               'href="/assets/favicon-32.png">')
    head.append("<style>%s%s</style></head><body><div class=\"wrap\">" % (CSS, _CHAIN_CSS))
    ...
    for i, (stage_name, session, result, rankings, state) in enumerate(stage_entries, 1):
        ...
        if state == "queued":
            ...
            continue
        if session is None:
            body.append('<p class="note">Running &mdash; no output yet.</p>')
            continue
        body.append(_tiles(session, result, rankings))
        body.append(_progress(session))
        body.append(_meta_summary(session, result))
    ...
    body.append("<footer>Generated by Roundtable.</footer>")
    body.append(_COPY_SCRIPT)
    return "\n".join(head + body) + "\n</div></body></html>\n"
